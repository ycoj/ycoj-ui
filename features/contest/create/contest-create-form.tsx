...
type Props = {
  defaultValues: ContestCreateFormValues;
  canAutoHide: boolean;
  domainId: string;
};
...
export default function ContestCreateForm({
  ...
  } = useForm<ContestCreateFormValues>({
  ...
  const onSubmit = async (values: ContestCreateFormValues) => {
    try {
      ...
      router.push(`/contest/${response.tid}`);
      router.refresh();
    } catch (error) {
...
type ContestTimingFieldsProps = {
  control: Control<ContestCreateFormValues>;
  register: UseFormRegister<ContestCreateFormValues>;
  errors: FieldErrors<ContestCreateFormValues>;
