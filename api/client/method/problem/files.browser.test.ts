...
describe('uploadProblemFile', () => {
  ...
  afterEach(() => {
    ...
    vi.restoreAllMocks();
    delete processEnv.NEXT_PUBLIC_UPLOAD_BASEURL;
    vi.unstubAllGlobals();
  });

  test('uploads directly with the frontend session', async () => {
    processEnv.NEXT_PUBLIC_UPLOAD_BASEURL =
  ...
  test('constructs config.yaml as a testdata upload', async () => {
  ...
  test('refreshes file lists and resolves direct download URLs', async () => {
    const fetchMock = vi
      .fn()
  ...
  test.each(['', 'nested/file.in', 'nested\\file.in', '.', '..'])(
